...
//the buttons
var next,forward,yes,no,home;
// backgrounds
var ground,space,bg2,bg,back;
//health 
var health;
...
var isChecked,assembled,planet;
...
function draw() {
 ...
 if(gameState === "tutorial"){
   ...
   speech(0,"Hello Obstinatel!! Welcome to 'NO WAY OUT!!' ..");
 ...
 if(rocket.visible === true && gameState === "start"){
   ...
   if(distance > 50000){
     ...
     if(planet.y > -200){
       ...
       speechBubble.visible = true;
       tutorial.style.display  = "inline-block";
       speech(31,"Here's your destination . . . . 'PLANET - kL103'. Press enter to land on it");
       if(keyCode === 13){
         gameState = "landed";
       }
     }
   ...
   if(gameState === "start" && planet.y <-200){
      if(health>0){
        if(ground.y >= -830){
          spawnObstacles();
        }
